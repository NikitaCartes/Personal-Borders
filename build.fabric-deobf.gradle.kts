...
// Tags make [fabric."26.1"] keys resolve via bare property("...").
stonecutter {
...
// The 26.1+ runtime is Mojang-mapped, so the accesswidener is in the `official` namespace.
...
dependencies {
    // Mojang-mapped, so no `mappings(...)` line.
...
tasks.processResources {
    ...
    // Mojang-mapped runtime needs no NeoForge metadata, and only one accesswidener is shipped.
